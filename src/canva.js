...
class CanvasSenpai {
  async welcome(member, { link, gradiant, blur } = {}) {
    ...
    ctx.font = `bold 42px Life`;
    ctx.fillstyle = `#F8F8F8`;
    ctx.fillText('Selamat Datang 🎉', 245, 55);
    ...
    ctx.fillText(`${name}#${member.user.discriminator}`, 278, 113);
    ctx.strokeText(`${name}#${member.user.discriminator}`, 278, 113);

    ctx.font = `bold 20px Life`;
    ctx.fillStyle = "#F8F8F8";


    ctx.fillText(`kamu adalah member ke-${member.guild.memberCount}`, 278, 160);
